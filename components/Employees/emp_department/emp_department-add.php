...
if ($cmd == 'edit' && isset($id)) {
	$sql_ee 			= "	SELECT a.* FROM employeedepartment a WHERE a.id = '" . $id . "' AND a.subscriber_users_id = '" . $subscriber_users_id . "'   ";
...
if (isset($is_Submit) && $is_Submit == 'Y') {
	...
	if (isset($department) && $department == "") {
		...
		$department_valid = "invalid";
	}

	if (empty($error)) {
		if ($cmd == 'add') {
			$sql_ee 			= "	SELECT a.* FROM employeedepartment a 
									WHERE a.subscriber_users_id 	= '" . $subscriber_users_id . "' 
									AND a.department				= '" . $department . "' ";
			$result_ee 			= $db->query($conn, $sql_ee);
			$counter_ee			= $db->counter($result_ee);
			if ($counter_ee == 0) {
				$sql = "INSERT INTO employeedepartment(subscriber_users_id, department,  add_date, add_by, add_ip)
						VALUES('" . $subscriber_users_id . "', '" . $department . "', '" . $add_date . "', '" . $_SESSION['username'] . "', '" . $add_ip . "')";
				// echo $sql;
				$ok = $db->query($conn, $sql);
				if ($ok) {
					$department = "";
					$msg['msg_success'] = "Department has been added successfully.";
				} else {
					$error['msg'] = "There is Error, Please check it again OR contact Support Team.";
				}
			} else {
				$error['msg'] = "This Department is already exist.";
			}
		} else if ($cmd == 'edit') {
			check_id($db, $conn, $id, "employeedepartment", $subscriber_users_id, $selected_db_name);
			$sql_ee 			= "	SELECT a.* FROM employeedepartment a 
									WHERE a.subscriber_users_id	= '" . $subscriber_users_id . "' 
									AND a.department			= '" . $department . "'
									AND a.id 				   != '" . $id . "'";
			...
			$counter_ee			= $db->counter($result_ee);
			if ($counter_ee == 0) {
				$sql_c_up = "UPDATE employeedepartment SET 	department		= '" . $department . "', 
					...
					<form method="post" autocomplete="off" enctype="multipart/form-data">
						...
						<div class="row">
							<div class="input-field col m12 s12">
								...
								<input id="department" type="text" name="department" value="<?php if (isset($department)) {
																								echo $department;
																							} ?>" required="" class="validate <?php if (isset($department_valid)) {
																																	echo $department_valid;
																																} ?>">
