...
$sql_cl 				= "	SELECT a.* FROM warranty a WHERE a.subscriber_users_id = '" . $subscriber_users_id . "' ORDER BY a.enabled DESC, a.warrantycode ";
//echo $sql_cl;
$result_cl 				= $db->query($conn, $sql_cl);
$count_cl 				= $db->counter($result_cl);
$page_heading 			= "Warranty Codes";
?>
<!-- BEGIN: Page Main-->
													...
													if ($count_cl > 0) {
														...
														foreach ($row_cl as $data) {
															...
															<tr data-id="<?php echo $id; ?>">
																<td><?php echo $i + 1; ?></td>
																<td><?php echo $data['warrantycode']; ?></td>
																<td><?php if ($data['laborwarranty'] == 1) { ?>Yes<?php } else { ?> No<?php } ?></td>
																<td><?php if ($data['partswarranty'] == 1) { ?>Yes<?php } else { ?> No<?php } ?></td>
