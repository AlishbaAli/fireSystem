...
if ($cmd == 'edit') {
	$title_heading = "Edit City";
	$button_val = "Edit";
}
if ($cmd == 'add') {
	$title_heading 	= "Add City";
	$button_val 	= "Add";
	$id 			= "";
}
if ($cmd == 'edit' && isset($id)) {
	$sql_ee 			= "	SELECT a.* FROM city a WHERE a.id = '" . $id . "' AND a.subscriber_users_id = '" . $subscriber_users_id . "'   ";
...
if (isset($is_Submit) && $is_Submit == 'Y') {
	...
	if (isset($city) && $city == "") {
		$error['msg'] = "Enter City Name";
		$city_valid = "invalid";
	}

	if (empty($error)) {
		if ($cmd == 'add') {
			$sql_ee 			= "	SELECT a.* FROM city a 
									WHERE a.subscriber_users_id 	= '" . $subscriber_users_id . "' 
									AND a.city 						= '" . $city . "' ";
			$result_ee 			= $db->query($conn, $sql_ee);
			$counter_ee			= $db->counter($result_ee);
			if ($counter_ee == 0) {
				$sql = "INSERT INTO city(subscriber_users_id, city,  add_date, add_by, add_ip)
						VALUES('" . $subscriber_users_id . "', '" . $city . "', '" . $add_date . "', '" . $_SESSION['username'] . "', '" . $add_ip . "')";
				// echo $sql;
				$ok = $db->query($conn, $sql);
				if ($ok) {
					$city = "";
					$msg['msg_success'] = "City has been added successfully.";
				} else {
					$error['msg'] = "There is Error, Please check it again OR contact Support Team.";
				}
			} else {
				$error['msg'] = "This City is already exist.";
			}
		} else if ($cmd == 'edit') {
			check_id($db, $conn, $id, "city", $subscriber_users_id, $selected_db_name);
			$sql_ee 			= "	SELECT a.* FROM city a 
									WHERE a.subscriber_users_id	= '" . $subscriber_users_id . "' 
									AND a.city 					= '" . $city . "'
									AND a.id 				   != '" . $id . "'";
			...
			$counter_ee			= $db->counter($result_ee);
			if ($counter_ee == 0) {
				$sql_c_up = "UPDATE city SET 	city			= '" . $city . "', 
						...
						<div class="row">
							<div class="input-field col m12 s12">
								...
								<input id="city" type="text" name="city" value="<?php if (isset($city)) {
																					echo $city;
																				} ?>" required="" class="validate <?php if (isset($city_valid)) {
																														echo $city_valid;
																													} ?>">
